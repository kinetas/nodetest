...
class _RequestedMissionScreenState extends State<RequestedMissionScreen> {
  List<Map<String, dynamic>> requestedMissions = [];
  bool isLoading = true;

  @override
  void initState() {
    super.initState();
    fetchRequestedMissions();
  }

  Future<void> fetchRequestedMissions() async {
    try {
      final response = await SessionTokenManager.get(
        'http://13.125.65.151:3000/nodetest/api/missions/missions/created_req',
      );
  ...
  void _showMissionDialog(BuildContext context, Map<String, dynamic> mission) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        title: Text("미션 처리", style: TextStyle(color: Colors.lightBlue[800])),
        ...
        actions: [
          TextButton(
            ...
            child: Text("성공", style: TextStyle(color: Colors.green)),
          ),
          TextButton(
            onPressed: () {
              Navigator.pop(context);
            ...
            child: Text("실패", style: TextStyle(color: Colors.red)),
          ),
        ],
      ),
    );
  ...
  void _showConfirmationDialog(BuildContext context, Map<String, dynamic> mission, String result) {
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        title: Text("확인", style: TextStyle(color: Colors.lightBlue[800])),
        ...
        actions: [
          TextButton(
            ...
            child: Text("예", style: TextStyle(color: Colors.green)),
          ),
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: Text("아니오", style: TextStyle(color: Colors.red)),
          ),
        ],
      ),
    );
  ...
  Future<void> _processMission(Map<String, dynamic> mission, String result) async {
    final url = result == '성공'
        ? 'http://13.125.65.151:3000/nodetest/api/missions/successMission'
        : 'http://13.125.65.151:3000/nodetest/api/missions/failureMission';

    final requestData = {'m_id': mission['m_id']};
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          requestedMissions.remove(mission);
        });
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
